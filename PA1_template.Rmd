...
(S)he takes on average `r format(mean_daily_total_steps, scientific = FALSE)` steps each day. The median is slightly smaller at `r median_daily_total_steps`. This is in excess of the often quoted [10 000 step healthy goal for each day](http://www.nhs.uk/Livewell/loseweight/Pages/10000stepschallenge.aspx).
...
This person is most active on average during the 5-minute interval that starts at `r most_active_interval`. This is during the time that (s)he most probably is walking to work in the morning.
...
A total of `r round(percent_na, 1)`% values are missing. For the purposis of this analysis they are imputed by the daily averages for a particular interval.
...
Based on the numbers above, the effect of imputing is very small in this case. The mean didn't change at all (difference is `r mean_diff` steps) and the median increased only slightly by `r round(median_diff, 2)` steps. This seems to indicate that the missing values were distributed relatively evenly throughout the day.
